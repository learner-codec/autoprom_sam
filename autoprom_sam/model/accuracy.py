...
def valid_fn(data_loader, model,batch_size):
    det = 0
    ...
    #confusionmat 
    confusion_mat = np.zeros([CFG.num_classes,CFG.num_classes])
    for iter_num,(id,img,mask,inst_mask,annot) in enumerate(tk0):
        with torch.no_grad():
            scores, classification, transformed_anchors = model(torch.stack(img).cuda().float())

        pred_boxes, label_list,scores_list = filter(scores.cpu(), classification.cpu(), transformed_anchors.cpu())
        #works with a single batch onlyz
        orig_boxes = annot[0][:,:4]
        orig_label = annot[0][:,4]
        if len(pred_boxes) ==0:
            print("no prediction made")
            pred_boxes = []
        c_ = get_confusion_matrix(orig_boxes,orig_label,pred_boxes,scores_list,label_list)
        confusion_mat+=c_
        del img,annot,mask
        gc.collect()
    #get f1 score 
    f1 = get_f1(confusion_mat,CFG.num_classes)
    return f1
...
def get_confusion_matrix(targets,orig_label,pred_boxes,scores_list,label_list):


    orig_b = torch.tensor(targets).int()
