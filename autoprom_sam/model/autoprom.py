...
class MaskHead(nn.Module):
    ...
    def forward(self,batched_input,image_embeddings):
        ...
        binarized_logits = torch.stack([torch.clamp(torch.sum(outputs[j]['low_res_logits'].sigmoid(),axis=0),0,1) for j in range(len(outputs))], axis=1)
...
class E2E(nn.Module):
    def __init__(self, num_classes, use_dense=True,attach_seg_head = True,train_bbox_decoder=True,train_seg_decoder=False,device="cuda"):
        ...
        self.detector = AutoProm(num_classes,use_dense=CFG.use_dense)
